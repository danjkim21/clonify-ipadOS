...
window.onload = () => {
    ...
    xhr.open('GET', 'https://api.openweathermap.org/data/2.5/weather?id=4140963&units=imperial&appid=d2e4d541802b524195b9b8f1544ef756');
    xhr.onload = () => {
        if (xhr.readyState === 4) {
            if (xhr.status === 200) {
                var prefix = "wi-";
...
//They are referenced the 
 function showCursor(e) { //Function to show/hide the cursor
...
function hideCursor(e) {    if(cursor.classList.contains('cursorVisible')) {
    cursor.classList.remove('cursorVisible');
    }
    cursor.classList.add('cursorHidden');
}

function mousemove(e) { //Function to correctly position the cursor
